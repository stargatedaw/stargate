...
SMS :: SMS(SMS *lo, int N, int band, int bandMax, int h, int res, int N0, int N1, int N2, int channels, audio *peak2)
{
  ...
  adjust2time = 0;
  adjust1time = 0;
  trial2GrainBuf = new GrainBuf(N,h,N1,hannpoisson,channels==1);
  trial1GrainBuf = new GrainBuf(N,h,N0,hannpoisson,channels==1);
                   ...
                   Slice **slices)
{
  ...
  for(int c=0; c<channels; c++) {    
    ...
    if(channels == 2) {
      ...
      memcpy(x1[0],g->x,(N+2)*sizeof(float));
    }
    float *mag1 = _mag1[c];
    calcmags(mag1, x1[c]);
#ifdef MULTITHREADED
...
void SMS :: add(grain *g0, grain *g1, grain *g2, int c)
{
  if(channels == 2) {
    ...
    if(band >= minTrial1Band) {
      memcpy(x00[0],g0->x,(N+2)*sizeof(float));
    }
    if(band >= minTrial2Band) {
      memcpy(x10[0],g1->x,(N+2)*sizeof(float));
    }
    memcpy(x2[0],g2->x,(N+2)*sizeof(float));
  }
...
Track *SMS :: createTrack(int c, TrackPoint *tp, const TimeType &time, bool bStitch) 
{
  ...
  Track *t = new Track((float)h1,index,tp,time,bStitch);
