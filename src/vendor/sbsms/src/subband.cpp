...
SubBand :: SubBand(SubBand *parent, int band, int channels, SBSMSQuality *quality, bool bSynthesize)
{
  ...
  if(sub) {
    ...
    grainsIn = new GrainBuf(NDownSample, NDownSample/SDownSample, NDownSample, hann,false);
    downSampledGrainAllocator = new GrainAllocator(NDownSample/2,NDownSample/2,hann,false);
  }
  if(band >= minTrial1Band) {
    grains[0] = new GrainBuf(N, h, N0, hannpoisson,channels==1);
  } else {
    grains[0] = NULL;
  }
  if(band >= minTrial2Band) {
    grains[1] = new GrainBuf(N, h, N1, hannpoisson,channels==1);
  } else {
    grains[1] = NULL;
  }
  grains[2] = new GrainBuf(N, h, N2, hannpoisson,channels==1);
  for(int c=0; c<channels; c++) {
    if(band >= minTrial1Band) {
      analyzedGrains[0][c] = new GrainBuf(N, h, N0, hannpoisson,channels==1);
    } else {
      analyzedGrains[0][c] = NULL;
    }
    if(band >= minTrial2Band) {
      analyzedGrains[1][c] = new GrainBuf(N, h, N1, hannpoisson,channels==1);
    } else {
      analyzedGrains[1][c] = NULL;
    }
    analyzedGrains[2][c] = new GrainBuf(N, h, N2, hannpoisson,channels==1);
  }
#ifdef MULTITHREADED
